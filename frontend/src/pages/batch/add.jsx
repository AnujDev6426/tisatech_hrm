...
function BatchAdd() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        if (res?.data?.status) {
          console.log("234567890-9876543", res?.data?.result?.UserList);
  ...
  useEffect(() => {
    ...
    if (formik?.values?.start_date || courseDetails?.duration) {
      ...
      endDate.setMonth(endDate.getMonth() + parseInt(courseDetails.duration));
      formik.setFieldValue("start_date", startDate.toISOString().split("T")[0]); // Format as YYYY-MM-DD
            ...
            <div className="row g-3 g-xl-5 g-xxl-5">
              ...
              <div className="col-sm-6 col-md-6 col-lg-6 col-xl-6 col-xxl-4 leave_type">
                ...
                >
                  ...
                  {employeeData?.map((res, i) => (
                    <option value={res?.username} key={i}>
                      {res?.username}
                    </option>
                  ))}
                </select>
